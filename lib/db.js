...
function insertCursorDiskKV(dbWrapper, key, value) {
	try {
		dbWrapper.db.run("INSERT OR IGNORE INTO cursorDiskKV (key, value) VALUES (?, ?)", [key, value]);
		return Promise.resolve();
	} catch (err) {
		return Promise.reject(err);
	}
...
function upsertCursorDiskKV(dbWrapper, key, value) {
	try {
		dbWrapper.db.run("INSERT OR REPLACE INTO cursorDiskKV (key, value) VALUES (?, ?)", [key, value]);
		return Promise.resolve();
	} catch (err) {
		return Promise.reject(err);
	}
