...
function execSqlite3(dbPath, sql) {
	...
	try {
		// Use file-backed stdio to avoid stdout maxBuffer limits for large query results.
...
/**
 * Run sqlite3 synchronously without maxBuffer limits by redirecting stdout/stderr to temp files.
 */
	...
	try {
		...
		try {
			...
			if (result.signal) {
				...
				const msg = stderrText || `sqlite3 terminated by signal ${result.signal}`;
				throw new Error(msg);
			}
			if (typeof result.status === 'number' && result.status !== 0) {
				const stderrText = fs.readFileSync(stderrPath, 'utf8').trim();
				const msg = stderrText || `sqlite3 exited with code ${result.status}`;
				throw new Error(msg);
			}
		} finally {
			fs.closeSync(outFd);
