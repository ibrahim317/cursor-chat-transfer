...
async function quickPickWorkspaceDbOrBrowse() {
	...
	const items = candidates.map(c => {
		...
		const label = hasDuplicates && folderPath ? folderPath : name;
		const description = hasDuplicates ? `(${name})` : (folderPath || c.hash);
		const detail = new Date(c.mtime).toLocaleString();
		return { label, description, detail, path: c.path, folderPath };
	});
	...
	const pick = await vscode.window.showQuickPick(items, {
		...
		placeHolder: 'Choose the workspace - full paths shown for duplicates'
	});
