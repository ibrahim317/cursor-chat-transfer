...
function getCurrentWorkspaceStateDbUri() {
	try {
		...
		// Fallback: basename match (for backwards compatibility, but prefer most recent)
...
async function quickPickWorkspaceDbOrBrowse() {
	...
	// Get current workspace folder path to detect which one is currently open
	let currentWorkspacePath;
	try {
		const folders = vscode.workspace.workspaceFolders;
		if (folders && folders.length > 0) {
			currentWorkspacePath = folders[0].uri.fsPath;
		}
	} catch {}
	
	// Check for duplicate basenames to warn users
	const nameCount = {};
	for (const c of candidates) {
		const name = tryGetWorkspaceName(c.hash);
		if (name) {
	...
	const items = candidates.map(c => {
		...
		const folderPath = tryGetWorkspaceFolderPath(c.hash);
		// If there are duplicates, emphasize the full path in the label
		const hasDuplicates = nameCount[name] > 1;
		
		// Check if this is the currently open workspace
		const isCurrent = currentWorkspacePath && folderPath === currentWorkspacePath;
		
		let label = hasDuplicates && folderPath ? folderPath : name;
		if (isCurrent) {
			label = `$(folder-opened) ${label}`;
		}
		
		let description = hasDuplicates ? `(${name})` : (folderPath || c.hash);
		if (isCurrent) {
			description = `(Current Directory) ${description}`;
		}
		
		const detail = new Date(c.mtime).toLocaleString();
		return { label, description, detail, path: c.path, folderPath, isCurrent };
	});
	
	// Sort to put current directory at the top
	items.sort((a, b) => {
		if (a.isCurrent && !b.isCurrent) return -1;
		if (!a.isCurrent && b.isCurrent) return 1;
		return 0; // Keep original order (most recent) for non-current items
	});
	...
	const pick = await vscode.window.showQuickPick(items, {
		...
		placeHolder: 'Choose the workspace - current directory shown at top'
	});
